        ...
        <div class="table-responsive">
            <table class="table nomargin">
                <thead>
                    <tr>
                        <th>
                        <span class="header-table-text">SUBSECRETARIAS</span>
                ...
                <tbody>
                    <tr v-for="item in data.data">
                        <td class="text-left" @click="$root.show(item.subsecretaria_id)">@{{ item.nombre }}</td>
                        <td class="text-left" @click="$root.show(item.subsecretaria_id)">
                            <ul v-for="sub in item.subsecretarias">
                                <li>@{{ sub.nombre }}</li>
                            </ul>
                        </td>
                        <td class="text-right" @click="$root.show(item.subsecretaria_id)">@{{ item.programas_count }}</td>
                        <td class="text-left" @click="$root.show(item.subsecretaria_id)">@{{ item.estado ? item.estado.descripcion : '' }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script type="text/x-template" id="button-create-template">
    <a href="{{ url('subsecretarias/create') }}" class="btn btn-default btn-add"><i style="margin-right: 5px" class="fa fa-plus"></i>Agregar @{{ $root.title }}</a>
</script>
  	...
  	let App = Vue.createApp({
    	...
    	methods: {
            ...
            show: function(id, event){
                window.location = "{{ url('subsecretarias') }}"+'/'+id+'/edit';
            },
